#!/bin/bash
# generate-workspace-templates-lambda-func.sh
set -e
...
# Remove old zip
rm -f "$ARTIFACTS_DIR/workspace-templates-lambda-function.zip"
...
zip -r "$ARTIFACTS_DIR/workspace-templates-lambda-function.zip" .
cd ..

rm -rf package venv
echo "Created workspace-templates-lambda-function.zip"
