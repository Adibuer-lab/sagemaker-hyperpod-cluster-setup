...
def handler(event, context):
    ...
    target_state_machine = os.environ.get("TARGET_STATE_MACHINE_ARN")
    if not queue_url or not (target_function or target_state_machine):
        raise ValueError("DLQ_URL and TARGET_FUNCTION or TARGET_STATE_MACHINE_ARN are required")
            ...
            try:
                if target_state_machine:
                    sfnclient.start_execution(
                        stateMachineArn=target_state_machine,
