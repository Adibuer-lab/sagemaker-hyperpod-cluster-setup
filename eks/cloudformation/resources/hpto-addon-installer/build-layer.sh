...
# Build script for HPTO addon Lambda layer
# This layer includes kubectl and AWS IAM authenticator for EKS operations

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAYER_DIR="${SCRIPT_DIR}/hpto-addon-lambda-layer"
...
echo "Building HPTO addon Lambda layer..."
...
zip -r "${SCRIPT_DIR}/hpto-addon-lambda-layer.zip" .

echo "Layer build complete: ${SCRIPT_DIR}/hpto-addon-lambda-layer.zip"
